...
if __name__ == '__main__':
    ...
    if action.lower().strip() == 'augment':
        ...
        process = subprocess.run(['DatasetAugmentation/.venv/bin/python3.11', './start_augmentation_train.py'], stdout=log, stderr=err)
        log.close()
        err.close()
        ...
        process = subprocess.run(['EEGClassificator/.venv/bin/python3.11', './start_classifier_train.py'], stdout=log, stderr=err)
        log.close()
        err.close()
